...
@Component({
  ...
  template: `
    <div class="fixed inset-0 bg-gray-900/40 backdrop-blur-sm z-50 flex justify-center items-center p-4 animate-fade-in-fast">
      <div class="bg-white rounded-lg shadow-xl p-6 w-full max-w-md text-center transform animate-slide-in-down">
        ...
        <div class="mt-6 flex justify-center gap-4">
          <app-button variant="outline" (click)="resolve(false)">Cancelar</app-button>
          <app-button variant="destructive" (click)="resolve(true)">Sí, confirmo</app-button>
        </div>
      </div>
    </div>
  `
