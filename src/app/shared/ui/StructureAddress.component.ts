...
@Component({
  ...
  providers: [{ provide: NG_VALUE_ACCESSOR, useExisting: forwardRef(() => StructuredAddressComponent), multi: true }],
  template: `
...
export class StructuredAddressComponent implements ControlValueAccessor {
  ...
  viaTypes: ListItem[] = [
    { id: 'Calle', code: 'Calle', name: 'Calle', order: 1 }, { id: 'Carrera', code: 'Carrera', name: 'Carrera', order: 2 },
    { id: 'Avenida', code: 'Avenida', name: 'Avenida', order: 3 }, { id: 'Transversal', code: 'Transversal', name: 'Transversal', order: 4 },
    { id: 'Diagonal', code: 'Diagonal', name: 'Diagonal', order: 5 },
  ];
  onChange: (value: any) => void = () => {};
  onTouched: () => void = () => {};
  constructor(private fb: FormBuilder) {
    this.addressForm = this.fb.group({
      viaType: ['Calle'], viaNumber: [''], crossStreetNumber: [''], houseNumber: ['']
    });
  }
  writeValue(value: any): void { if (value) { this.addressForm.setValue(value, { emitEvent: false }); } }
  registerOnChange(fn: any): void {
    this.onChange = fn; this.sub?.unsubscribe();
    this.sub = this.addressForm.valueChanges.subscribe(fn);
  }
  registerOnTouched(fn: any): void { this.onTouched = fn; }
  setDisabledState(isDisabled: boolean): void { isDisabled ? this.addressForm.disable() : this.addressForm.enable(); }
}
